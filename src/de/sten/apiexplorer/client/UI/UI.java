...
public class UI {
	...
	public void completeFormWithClickPath(String clickPath, ArrayList<Request_Template> templates) {
		...
		if (currentlySelectedApiMethod == null || !(currentlySelectedApiMethod.equals(method)))
			preselectMethod(api, method);
		currentlySelectedApiMethod = method;
		if (clickpath.length < 3)
			return;
		...
		if (parametertype.equals("Header Parameters"))
			isHeader = true;
		if (clickpath.length < 4)
			return;
	...
	private void preselectMethod(Request_Template api, APIMethod method) {
		...
		for (int i = 0; i < methodchsr.getItemCount(); i++) {
			if (method.getMethodtype().equals(methodchsr.getItemText(i)))
				methodchsr.setSelectedIndex(i);
		}
